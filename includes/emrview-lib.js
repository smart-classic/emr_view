...
EMRView.canvas.createAppWidgetBefore = function(app_id,title,prepend_id){
	...
	jQuery("<li id='"+appwidget_id+"' class='widget widget-color ui-emr-widget'><div class='widget-head'><a href='#' class='zoom'>Zoom</a> <a href='#' class='close'>Close</a><h3>"+title+"</h3></div><div class='widget-content ui-state-disabled'><iframe SEAMLESS src='about:blank' id='frame"+appwidget_id+"' class='iframemini' width='100%' height='100%' border='0' frameborder='0'> </iframe></div></li>").hide().insertBefore("#"+prepend_id).show('drop', {}, 500);
...
EMRView.canvas.createAppWidgetAppend = function(app_id,title,append_id){
        var appwidget_id = createAppWidgetId(app_id);
        jQuery("<li id='"+appwidget_id+"' class='widget widget-color ui-emr-widget'><div class='widget-head'><a href='#' id='zoomlink"+appwidget_id+"' onclick=\"javascript:EMRView.canvas.zoomAppWidget('"+appwidget_id+"');\" class='zoom'>Zoom</a> <a href='#' class='close'>Close</a><h3>"+title+"</h3></div><div class='widget-content ui-state-disabled'><div class='widget-cover' style='display:none; position: absolute; float:left; min-height: 100%; height: 100%; width: 100%; '></div><iframe SEAMLESS src='about:blank' id='frame"+appwidget_id+"' class='iframemini' width='100%' height='100%' border='0' frameborder='0'> </iframe></div></li>").hide().appendTo("#"+append_id).show();
...
EMRView.canvas.loadCanvasId = function(canvas_id){
	...
	$(data).find('canvas').each(function(){
		if($(this).attr('id') == canvas_id){ // found canvas_id
			...
			if(EMRViewDebugMode) alert("start unbinding click events");
			...
			if(EMRViewDebugMode) alert("finished unbinding click events");
		...
		$(this).find('view').each( function(){
			...
			if($(this).attr('default') == 'true'){
				canvas_default = canvas_id;
                                ...
                                $("<div><span style='float:right;margin-top:3px;cursor:move;display:none;'><img src='assets/img/handle.png'/></span><h4 id='view-" + canvas_id + "' class='ui-widget-content ui-state-hover'><a href='#' onclick='javascript:EMRView.canvas.loadCanvasId\(\"" + canvas_id + "\"\);return false;'>" + canvas_name + "</a></h4></div>").appendTo('#emrview-views');
			}
			else {
				$("<div><span style='float:right;margin-top:3px;cursor:move;display:none;'><img src='assets/img/handle.png'/></span><h4 id='view-" + canvas_id + "' class='ui-widget-content'><a href='#' onclick='javascript:EMRView.canvas.loadCanvasId\(\"" + canvas_id + "\"\);return false;'>" + canvas_name + "</a></h4></div>").appendTo('#emrview-views');
			}
		});
        });
